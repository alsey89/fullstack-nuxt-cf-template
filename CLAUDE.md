...
- Frontend uses the error `code` to determine what to show users (via i18n)
- The `message` parameter is for development/debugging only
- Error codes are defined in `shared/error/codes.ts`
...
See `server/CLAUDE.md` for detailed error handling patterns.
